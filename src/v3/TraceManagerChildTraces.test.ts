...
describe('TraceManager - Child Traces (Nested Proposal)', () => {
  ...
  describe('Edge Cases', () => {
    ...
    it('should skip waiting-for-interactive state for children', () => {
      ...
      traceManager.when('state-transition').subscribe((event) => {
        console.log('trace', event.traceContext.input.id, event.stateTransition.transitionToState)
      })
      traceManager.when('add-span-to-recording').subscribe((event) => {
        console.log('trace', event.traceContext.input.id, 'span', event.spanAndAnnotation.span.name, event.spanAndAnnotation.span.startTime.now)
      })
      const parentTracer = traceManager.createTracer({
        name: 'ticket.parent-operation',
        type: 'operation',
      ...
      // // prettier-ignore
      // const { spans } = getSpansFromTimeline<TicketIdRelationSchemasFixture>`
      //   Events: ${Render('parent-start', 0)}---${Render('parent-end', 0)}--${LongTask(300)}--${Render('child-end', 0)}--${LongTask(50)}}---${Check}
      //   Time:   ${0}                           ${125}                      ${200}            ${220}                     ${550}             ${5_000}
      // `
      ...
      expect(parentReport?.additionalDurations.startTillInteractive).toBe(550) // last long task starts at 200 + 300 duration
  ...
  describe('Integration Scenarios', () => {
    it('should handle child adoption with debouncing', () => {
      ...
      const parentTracer = traceManager.createTracer({
        ...
        requiredSpans: [{ name: 'parent-end' }],
        adoptAsChildren: ['ticket.child-operation'],
        debounceOnSpans: [matchSpan.withName('parent-debounce')],
        debounceWindow: 200,
        variants: {
          default: { timeout: DEFAULT_TIMEOUT_DURATION },
        },
      ...
      const childTracer = traceManager.createTracer({
        ...
        relationSchemaName: 'ticket',
        requiredSpans: [{ name: 'child-end' }],
        debounceOnSpans: [matchSpan.withName('child-debounce')],
        debounceWindow: 150,
        variants: {
          default: { timeout: DEFAULT_TIMEOUT_DURATION },
        },
      ...
      // prettier-ignore
      const { spans } = getSpansFromTimeline<TicketIdRelationSchemasFixture>`
      Events: ${Render('parent-start', 0)}---${Render('child-start', 0)}---${Render('parent-debounce', 0)}---${Render('child-debounce', 0)}---${Render('child-end', 0)}---${Render('parent-end', 0)}
      Time:   ${0}                           ${25}                         ${50}                            ${100}                           ${300}                     ${400}
      `
      ...
      expect(parentReport?.duration).toBe(400) // parent-start to parent-end
      expect(childReport?.duration).toBe(275) // child-start to child-end, accounting for debounce
      expect(reportErrorFn).not.toHaveBeenCalled()
    })
    ...
    it('should handle child adoption with computed spans', () => {
      ...
      // prettier-ignore
      const { spans } = getSpansFromTimeline<TicketIdRelationSchemasFixture>`
      Events: ${Render('parent-start', 0)}---${Render('child-start', 0)}---${Render('parent-middle', 50)}---${Render('child-middle', 50)}---${Render('child-end', 0)}---${Render('parent-end', 0)}
      ...
      expect(childReport?.computedSpans?.['child-computed']?.duration).toBe(100)
      expect(reportErrorFn).not.toHaveBeenCalled()
    })
